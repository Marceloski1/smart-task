...
export function RecommendationCard({ recommendation, task, onAccept, onReject }: RecommendationCardProps) {
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <Card className="border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-secondary/5">
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">{format(recommendation.recommendation_date, "MMMM d, yyyy")}</p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-start justify-between gap-2">
              <h4 className="text-lg font-semibold leading-relaxed">{task.title}</h4>
          ...
          <div className="rounded-lg bg-muted/50 p-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{recommendation.recommendation_reason}</p>
          </div>
